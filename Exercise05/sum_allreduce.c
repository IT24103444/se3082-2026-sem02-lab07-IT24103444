...
int main(int argc, char **argv) {
    ...
    /*
     * ALLREDUCE: Add all local sums together.
     * Every process receives the final total sum.
     */
    long long total_sum = 0;

    MPI_Allreduce(&local_sum, &total_sum, 1,
                  MPI_LONG_LONG, MPI_SUM,
                  MPI_COMM_WORLD);

    /* Every process can now use the global total */
